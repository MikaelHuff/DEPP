...
class encoder(nn.Module):
    def __init__(self, args, model_type='full', prev_linear=None):
...
class model(LightningModule):
    def __init__(self, args, model_type='full', prev_model=None):
        super(model, self).__init__()
        self.save_hyperparameters(args)
        if not self.hparams.sequence_length:
            utils.get_seq_length(self.hparams)
        self.save_hyperparameters(self.hparams)
        self.encoder = encoder(self.hparams, model_type, prev_model)
        self.channel = 4
